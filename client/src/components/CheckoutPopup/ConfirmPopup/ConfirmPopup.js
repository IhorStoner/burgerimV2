...
export default function ConfirmPopup({ handleClosePopup, type,orderNumber }) {
  const lng = useSelector(getLanguage)
  return (
    <div className='successPopup'>
      <div className="successPopup__modal">
        <p className='successPopup__text'>
          {type === 'success' && lng ==='RUS' && `Номер заказа ${orderNumber}. Благодарим за заказ,Мы свяжемся с Вами  в ближайшее время`}
          {type === 'success' && lng === 'UKR' && `Номер замовлення ${orderNumber}. Дякуємо за замовлення, Ми зв'яжемося з Вами найближчим часом`}
